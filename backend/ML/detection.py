    ...
    if iterate == False:
        ...
        for i in range(count):
            for j in range(i+1, count):
                x_new, y_new, _ = getNormalisedCoordinates(
                    x[i:], y[i:], i, j, lines)
...
def makeTemplates():
    ...
    template_directory = "C:\\Users\\heera\\OneDrive\\Documents\\GitHub\\CSSHack\\backend\\ML\\Templates"
    ...
    for filename in os.listdir(template_directory):
        ...
        img = cv2.imread(
            "C:\\Users\\heera\\OneDrive\\Documents\\GitHub\\CSSHack\\backend\\ML\\Templates\\" + filename)
        ...
        drawn_lines = cv2.HoughLinesP(final_lines, rho, theta, threshold, np.array(
        ...
        contours, hierarchy = cv2.findContours(
            edge_copy, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
        ...
        templates_coordinates[filename[:-4]
                              ] = (x, y, len(final_contours), normalised_lines)
    ...
    with open("C:\\Users\\heera\\OneDrive\\Documents\\GitHub\\CSSHack\\backend\\ML\\Template Coordinates", "wb") as fp:
...
def test_normaliser(test_path):
    ...
    img2 = getGrayscale(img)
    cv2.imshow('test_img', img2)

    a = 50
    thresh = binariseImage(img2, [190])
    # Subtracting to get only stars
    final = thresh[0]
    plotImage(final, "final")
...
def simillarity_error(train, test):
    ...
    for i in range(train[0].shape[0]):
        distances = np.sqrt((test[0] - train[0][i]) **
                            2 + (test[1] - train[1][i]) ** 2)
...
def test_runner(constellation):
    test_coordinates = test_normaliser(
        'C:\\Users\\heera\\OneDrive\\Documents\\GitHub\\CSSHack\\backend\\ML\\test_data\\' + constellation + '.png')
    true_label = constellation[:]
    # print(len(test_coordinates))

    file = open(
        'C:\\Users\\heera\\OneDrive\\Documents\\GitHub\\CSSHack\\backend\\ML\\Template Coordinates', 'rb')
    ...
    for bright_perm in range(len(test_coordinates)):
        for constellation in template_coordinate:
            x_template, y_template, n_stars, normalised_lines = template_coordinate[
                constellation]

            e = simillarity_error((x_template, y_template),
                                  test_coordinates[bright_perm])
            ...
            if e[0] > 2 and score < cur_score < 1e+3:
                ...
                plot_points = (x_template, y_template,
                               test_coordinates, normalised_lines)
